    ...
    public:
        ...
        static void MakeBloonByPosition(Layer::NormalBloonLayer type, Vector2 startPosition, int nowRouteTarget)
        {
            if (BloonPool.empty())
            {
                ...
                tmpBloon.SetTopLeft(static_cast<int>(startPosition.X), static_cast<int>(startPosition.Y));
            ...
            for (int i = 0; i < static_cast<int>(BloonVector.size()); i++)
            {
                ...
                if (BloonVector[i].IsPoped())
                {
                    ...
                    if (type == BloonType::black || type == BloonType::white)
                    {
                        ...
                        Vector2 position = {
                            static_cast<float>(BloonVector[i].GetLeft()), static_cast<float>(BloonVector[i].GetTop())
                        };
